...
| `three-item-spectrum` | 三项内容需要放在同一连续尺度上比较位置 | 只有明确高低顺序的排行 |
| `source-to-system` | 原始材料转化为模型、知识库或数字系统 | 普通文件整理或页面展示 |
| `split-image-window` | 两张真实图片代表新旧路径、两代产品或两种阵营 | 只有文字差异的普通对比 |
| `scale-grid` | 数字与重复单元共同表现规模、容量或并行数量 | 无法映射为数量或密度的抽象判断 |
| `ratio-bars` | 两项数值需要按真实比例展示差距 | 只有定性差异、没有可靠数值 |
| `one-to-many` | 一项能力替代或覆盖四项任务 | 普通四项并列清单 |
| `complexity-reduction` | 复杂输入被压缩为更短代码、流程或结论 | 前后复杂度没有明显变化 |
| `feature-proof-list` | 新产品、新模型或开源项目的已验证功能清单 | 未核验的营销卖点 |
| `unlock-breakthrough` | 解除锁定、突破限制并出现多个新选择 | 风险仍在累积的闭锁状态 |
